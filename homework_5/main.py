...
uniqueData = data_set['shrimp'].append(data_set['almonds']).append(data_set['avocado']).append(data_set['vegetables mix']).append(
    data_set['green grapes']).append(data_set['whole weat flour']).append(data_set['yams']).append(data_set['cottage cheese']).append(
    data_set['energy drink']).append(data_set['tomato juice']).append(data_set['low fat yogurt']).append(data_set['green tea']).append(
...
list_data_with_spaces = data_set.values.tolist()

list_data_no_nan = []
for partData in list_data_with_spaces:
...
for i in range(len(prepared_data)):
    for j in range(len(prepared_data[i])):
        for k in range(len(uniqueData)):
            if (prepared_data[i][j] == uniqueData[k]):
                prepared_data[i][j] = k
...
def clean_bitmap():
    for i in list_doubletones:
        if len(i) > 1:
            for j in i:
                if (len(j) > 1):
                    if (j[0] + j[1]) % 100 in bit_map.keys() and (j[0] + j[1] * 2) % 100 in bit_map2.keys() and j[0] not in bad_numbers and j[1] not in bad_numbers:
...
for i in result:
    print(uniqueData[i[0]] + ", " + uniqueData[i[1]])